        ...
        <section v-if="(items || []).length" class="d-flex px-9 gap-12 flex-wrap flex-column">
          <div v-for="item in items" :key="item.id" 
            ...
            <section class="bg-secondary d-flex flex-grow justify-center brtl-5 brtr-5 py-2 flex-column align-center px-6" style="position: relative;">
              <span class="color-grey font-18--bold mb-2">{{ item.title }}</span>
              <div class="d-flex flex-wrap gap-3">
                ...
                <span
                  v-if="step == 'scheduled'"
                  class="color-primary font-14--bold hover-bg-primary hover-color-white pointer px-2 bra-4 transition-all py-1"
                  @click="finish(item._id)">
                  Finalizar
                </span>
                <span
                  v-if="['opened', 'finished', 'canceled'].includes(step)"
                  class="bg-red color-white font-14--bold hover-bg-primary hover-color-cyan pointer px-2 bra-4 transition-all py-1"
            ...
            <section class="d-flex align-center justify-center gap-2 py-2 font-18--bold flex-column color-white px-7 text-center flex-wrap">
              ...
              <v-menu v-if="item?.user" offset-y content-class="bg-primary bra-3">
                <template #activator="{ on }">
                  <div class="color-cyan bg-secondary font-14--bold hover-bg-white hover-color-primary pointer px-2 bra-4 transition-all py-1" v-on="on">
                    Detalhes
                  </div>
                </template>
                <div v-if="item?.user" class="d-flex flex-column bg-primary px-4 py-4 font-16--bold color-white">
                  <span class="color-cyan text-center font-18--bold mb-2">Paciente</span>
                  <span class="color-cyan">Nome: <span class="color-white">{{ item?.user?.name }}</span></span>
                  <span class="color-cyan">Email: <span class="color-white">{{ item?.user?.email }}</span></span>
                  <span class="color-cyan">Cpf: <span class="color-white">{{ item?.user?.cpf }}</span></span>
                </div>
              </v-menu>
            </section>
          </div>
...
export default {
  ...
  methods: {
    remove(id){
      this.confirm({text: 'Você deseja realmente excluir esse agendamento?'}, () =>{
        ScheduleService.remove(id).then(() => {
          this.alertSuccess({title: 'Cancelamento excluido com sucesso'})
    ...
    finish(id){
      this.confirm({text: 'Você deseja confirmar a finalização desse agendamento?'}, () =>{
        ScheduleService.toFinish(id).then(() => {
          this.alertSuccess({title: 'Agendamento finalizado com sucesso'})
          this.reload(this.step)
        })
      })
    },
    async reload(step){
      ...
      let values = [];
      if(step == 'opened'){
        values = await ScheduleService.opened() || []
      }
      else if(step == 'scheduled'){
        values = await ScheduleService.scheduledAdmin() || []
      }else if(step == 'canceled'){
        values = await ScheduleService.canceledAdmin() || []
      }else{
        values = await ScheduleService.finishedAdmin() || []
      }
